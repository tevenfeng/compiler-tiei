...
class ExpListNode extends ASTnode {
    ...
    public void typeCheck(List<Type> typeList) {
        for(int i = 0;i<myExps.size();i++){
            Type tmp = myExps.get(i).typeCheck();
            if(!tmp.isErrorType()){
                if(!tmp.equals(typeList.get(i))){
...
class DotAccessExpNode extends ExpNode {
     ...
     */
    public Type typeCheck() {
        if(!myLhs.typeCheck().isStructType()){
            return new ErrorType();
        }else{
            Type tmp = myId.typeCheck();
            if(!tmp.isErrorType()){
                return tmp;
            }else{
                return new ErrorType();
...
class AssignNode extends ExpNode {
     ...
     */
    public Type typeCheck() {
        Type ltmp = myLhs.typeCheck();
        Type rtmp = myRhs.typeCheck();
        if(ltmp.isErrorType()||rtmp.isErrorType()){
            return new ErrorType();
        }else{
            if(myLhs instanceof CallExpNode){
                ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Function assignment");
                return new ErrorType();
            }else if(ltmp.isFnType()||(!(myRhs instanceof CallExpNode)&&(rtmp.isFnType()))){
                ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Function assignment");
                return new ErrorType();
            }else if(ltmp.isStructType()||rtmp.isStructType()){
                ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(), "Struct variable assignment");
                return new ErrorType();
            }else if(!ltmp.equals(rtmp)){
...
class CallExpNode extends ExpNode {
     ...
     */
    public Type typeCheck() {
        if(myId.info() instanceof FnInfo){
            List<Type> paramTypes = ((FnInfo)myId.info()).getParamTypes();

            if(myExpList.size()!=paramTypes.size()){
                ErrMsg.fatal(myId.lineNum(), myId.charNum(), "Function call with wrong number of args");
            }else{
                myExpList.typeCheck(paramTypes);
            }
...
abstract class ArithmeticExpNode extends BinaryExpNode {
     ...
     */
    public Type typeCheck() {
        Type tmp1 = myExp1.typeCheck();
        Type tmp2 = myExp2.typeCheck();
        if((tmp1.isErrorType())||(tmp2.isErrorType())){
            return new ErrorType();
        }else{
            boolean flag = false;
            if((!tmp1.isIntType())){
