...
class FormalsListNode extends ASTnode {
    ...
    public List<Type> nameAnalysis(SymTable symTab) {
        ...
        for (FormalDeclNode node : this.myFormals) {
            SymInfo tmp = node.nameAnalysis(symTab);
            if(tmp!=null){
                typeList.addLast(tmp.getType());
            }
        }
        return typeList;
    }    
...
class VarDeclNode extends DeclNode {
    ...
    public SymInfo nameAnalysis(SymTable symTab) {
        if(this.myType.isVoidType()){
            ErrMsg.fatal(this.myId.lineNum(), this.myId.charNum(), "bad declarations");
            return null;
        }else if(this.myType.isStructType()){
            if(symTab.lookupGlobal(this.myId.name())==null){
                ErrMsg.fatal(this.myId.lineNum(), this.myId.charNum(), "uses of undeclared names");
                return null;
            }
        }

        if(symTab.lookupLocal(this.myId.name())!=null){
            ErrMsg.fatal(this.myId.lineNum(), this.myId.charNum(), "find multiply declared names");            
            return null;
        }else{
            symTab.addDecl(this.myId.name(), new SymInfo(this.myType));
            return null;
        }
    }
    ...
    public SymInfo nameAnalysis(SymTable structSymTab, SymTable globalTab) {
        if(this.myType.isVoidType()){
            ...
            if(this.myType.isStructType()){
                if(globalTab.lookupGlobal(this.myId.name())==null){
                    ErrMsg.fatal(this.myId.lineNum(), this.myId.charNum(), "uses of undeclared names");
                    return null;
                }
            }
        ...
        if(globalTab.lookupLocal(this.myId.name())!=null){
            ErrMsg.fatal(this.myId.lineNum(), this.myId.charNum(), "find multiply declared names");
            return null;
        }else{
            structSymTab.addDecl(this.myId.name(), new SymInfo(this.myType));
            return null;
        }
    }    
